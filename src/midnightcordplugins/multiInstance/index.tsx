...
const MI_TOKEN_CACHE_KEY = "midnightcord-mi-token-cache";
...
function ContextMenuPortal(props: CtxMenuProps) {
    ...
    const [container] = React.useState(() => {
        const el = document.getElementById("midnightcord-mi-ctx-root") ?? document.createElement("div");
        el.id = "midnightcord-mi-ctx-root";
...
export default definePlugin({
    ...
    async start() {
        ...
        addHeaderBarButton("midnightcord-multi-instance", () => <MultiInstanceButton />, 9);
    },

    stop() {
        removeHeaderBarButton("midnightcord-multi-instance");
        if (this._fluxUnsub) { this._fluxUnsub(); this._fluxUnsub = null; }
        const root = document.getElementById("midnightcord-mi-ctx-root");
        if (root) root.remove();
    },
});
