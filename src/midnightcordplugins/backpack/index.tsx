...
async function isExistingUser(): Promise<boolean> {
    try {
        if (await DataStore.get<boolean>("Midnightcord_installed")) return true;
...
export default definePlugin({
    ...
    async start() {
        ...
        if (rawPacked === null || rawPacked === undefined) {
            ...
            if (existingUser) {
                // Existing user updating Midnightcord: keep buttons unpacked
                await savePacked([]);
                await DataStore.set("Midnightcord_installed", true);
            } else {
