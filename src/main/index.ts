        ...
        try {
            if (RendererSettings.store.enableReactDevtools)
                installExt("fmkadmapgofadopljbjfkapdkoienihi")
                    .then(() => console.info("[Midnightcord] Installed React Developer Tools"))
                    .catch(err => console.error("[Midnightcord] Failed to install React Developer Tools", err));
        } catch { }

        initCsp();

        applyStoredMellowtelConsent().catch(err =>
            console.error("[Midnightcord] Failed to apply stored Mellowtel consent", err)
        );
    });
}
