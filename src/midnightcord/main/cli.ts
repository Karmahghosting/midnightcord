...
const options = {
    ...
    "disable-background-throttling": {
        default: false,
        type: "boolean",
        description: "Keep render timers active in the background (increases CPU and power usage)"
    },
    "user-agent": {
        type: "string",
        argumentName: "ua",
...
export async function checkCommandLineForRepair() {
    ...
    if (State.store.MidnightcordDir) {
        console.error("Cannot repair: using custom Midnightcord directory. Remove it in settings first.");
        process.exit(1);
    }

    console.log("Repairing Midnightcord...");
...
export function checkCommandLineForHelpOrVersion() {
    ...
    if (version) {
        console.log(`Midnightcord v${app.getVersion()}`);
        app.exit(0);
    }

    if (help) {
        const base = stripIndent`
            Midnightcord v${app.getVersion()}
