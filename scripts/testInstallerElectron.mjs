...
async function electronTest(config) {
    ...
    try {
        ...
        await waitFor("!document.getElementById('selection-view').hidden", "Uninstall tab did not open after installation");
        assert.equal(await evaluate("document.querySelectorAll('#targets input:not(:disabled)').length"), 1, "Completed install updates the target state before switching tabs");
        ...
        assert.equal(await evaluate("document.querySelectorAll('#targets input:not(:disabled)').length"), 0, "Restored installation is no longer removable");
